...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    if ([text isEqualToString:@""]) {
        ...
        QQChatViewController *vc = (QQChatViewController *)self.viewController;
        vc.textField.text = self.textView.text;
        // 自己发送一条消息
        [vc sendMessageWithText:self.textView.text messageType:QQChatTypeMe];
        
        // 对方回复一条消息
        NSString *replayText = [vc replyWithText:self.textView.text];
        [vc sendMessageWithText:replayText messageType:QQChatTypeOther];
